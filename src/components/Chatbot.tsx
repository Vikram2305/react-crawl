...
const CustomerSupportChatbot: React.FC<LLMmodelProps> = ({ apiKey, selectedOption }) => {
  ...
  const handleWebSocketMessage = (data: any) => {
    ...
    if (data.vehicle_models && data.vehicle_models.length > 0) {
    ...
    } else if (data.variant_information && data.variant_information.length > 0) {
      setMessages((prev) => [
        ...prev,
        {
      ...
      setMessages((prev) => [
        ...
        {
          type: "text",
          content: data.responses || "Make sure to select a Project from the dropdown. Please try again.",
          sender: "bot",
  ...
  const handleSubmitInterest = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://scrape-graph-api-dev.ispgnet.com/ingest/saveInterest", {
        method: "POST",
      ...
      setMessages((prev) => [
        ...
        {
          type: "text",
          content: "Sorry, there was an error saving your interest. Please try again later.",
          sender: "bot",
        },
      ])
    }
  ...
  const renderVehicleCard = (variant: VehicleVariant | ProductRecommendation) => {
    return (
      <div className="w-full bg-white text-black shadow-lg rounded-xl">
        <div className="p-4">
          <img
            src={variant.image_src || (variant as ProductRecommendation).image_src}
  ...
  const renderVehicleCards = (variants: VehicleVariant[], recommendations: ProductRecommendation[] | undefined) => {
    return (
                ...
                <div className="flex overflow-x-hidden">
                  {recommendations.map((recommendation, index) => (
                    <div
                      ...
                      className="flex-shrink-0 w-full transition-transform duration-300 ease-in-out"
                      style={{
                        transform: `translateX(-${currentRecommendationIndex * 100}%)`,
                      }}
                    >
                  ...
                  {recommendations.length > 1 && (
                    <>
                      <button
                        onClick={() =>
                          setCurrentRecommendationIndex((prev) => (prev > 0 ? prev - 1 : recommendations.length - 1))
                        }
                        className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black text-white rounded-full p-2"
                      ...
                      <button
                        onClick={() =>
                          setCurrentRecommendationIndex((prev) => (prev < recommendations.length - 1 ? prev + 1 : 0))
                        }
                        className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-black text-white rounded-full p-2"
  ...
  const renderVariantInformationTable = (variantInfo: VariantInformation) => {
    return (
      <div className="overflow-x-auto w-full">
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          <tbody>
            {Object.entries(variantInfo).map(([key, value]) => {
              if (typeof value === "object" && value !== null && key !== "image_src") {
                return (
                  <React.Fragment key={key}>
                    ...
                    {Object.entries(value).map(([subKey, subValue]) => (
                      <tr key={`${key}-${subKey}`}>
                        <td className="py-2 px-4 border-b font-bold">{greet(subKey.replace(/_/g, " "))}</td>
                        <td className="py-2 px-4 border-b">
                          {typeof subValue === "object" && subValue !== null
                            ? Object.entries(subValue).map(([subSubKey, subSubValue]) => (
                                <div key={subSubKey}>{`${subSubKey.replace(/_/g, " ")}: ${subSubValue}`}</div>
                              ))
                            : String(subValue)}
                        </td>
                      </tr>
                ...
                return (
                  <tr key={key}>
                    <td className="py-2 px-4 border-b">{key.replace(/_/g, " ")}</td>
        ...
        <div className="mt-4">
          <img
            src={variantInfo.image_src || "/placeholder.svg"}
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          <tbody>
            {Object.keys(variantInfo1)
              ...
              .map((key, index) => {
                if (typeof variantInfo1[key] === "object" && variantInfo1[key] !== null) {
                  return (
                    <React.Fragment key={key}>
                      <tr className="bg-gray-50">
                        <td className="py-2 px-4 border-b font-bold" colSpan={3}>
                          {key.replace(/_/g, " ").toUpperCase()}
                        </td>
                      </tr>
                      {Object.keys(variantInfo1[key]).map((subKey, subIndex) => (
                        <tr key={`${key}-${subKey}`}>
                          <td className="py-2 px-4 border-b font-bold">{greet(subKey.replace(/_/g, " "))}</td>
                          <td className="py-2 px-4 border-b">{renderCompareVariantValue(variantInfo1, key, subKey)}</td>
                          <td className="py-2 px-4 border-b">{renderCompareVariantValue(variantInfo2, key, subKey)}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  )
                  ...
                  return (
                    <tr key={key}>
                      <td className="py-2 px-4 border-b">{key.replace(/_/g, " ")}</td>
                      <td className="py-2 px-4 border-b">{variantInfo1[key].join(", ")}</td>
                      <td className="py-2 px-4 border-b">{variantInfo2[key].join(", ")}</td>
                    </tr>
                  )
                } else if (key !== "image_src") {
                  return (
                    <tr key={key}>
                      <td className="py-2 px-4 border-b">{key.replace(/_/g, " ")}</td>
                      <td className="py-2 px-4 border-b">{variantInfo1[key]}</td>
                      <td className="py-2 px-4 border-b">{variantInfo2[key]}</td>
                    </tr>
                  )
                }
                return null
        ...
        <div className="mt-4 flex justify-around">
          {variants.map((variant, index) => (
            <div key={index} className="w-1/3 px-2">
              <img
                src={variant.details[0].image_src || "/placeholder.svg"}
  ...
  const renderCompareVariantValue = (detail: any, key: string, subKey?: string) => {
    if (subKey) {
      if (typeof detail[key][subKey] === "object" && detail[key][subKey] !== null) {
        return Object.keys(detail[key][subKey]).map((subSubKey, subSubIndex) => (
          <div key={subSubKey}>{`${subSubKey.replace(/_/g, " ")}: ${detail[key][subKey][subSubKey]}`}</div>
        ))
      } else {
      ...
      if (typeof detail[key] === "object" && detail[key] !== null) {
        return Object.keys(detail[key]).map((subKey, subIndex) => (
          <div key={subKey}>{`${subKey.replace(/_/g, " ")}: ${detail[key][subKey]}`}</div>
        ))
    ...
    <div className="fixed bottom-4 right-4">
      {!isOpen && (
        <button
          ...
          className="fixed bottom-4 right-5 flex items-center rounded-full bg-black text-white py-2 px-4 transition-transform transition-colors duration-300 ease-in-out hover:scale-105"
        >
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex-grow overflow-y-auto p-4 space-y-4">
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                ...
                {msg.type === "vehicle_models" && msg.vehicle_models && renderVehicleModelButtons(msg.vehicle_models)}
                {msg.type === "vehicleVariants" &&
                  msg.vehicleVariants &&
                  renderVehicleCards(msg.vehicleVariants, msg.product_recommendation)}
                ...
                {msg.type === "form" && !isFormSubmitted && (
                  <form onSubmit={handleSubmitInterest} className="space-y-2 w-[80%]">
                    <input
                      type="text"
            ...
            {messages[messages.length - 1]?.sender === "bot" && messages[messages.length - 1]?.recommendations && (
              <div className="flex flex-wrap justify-end gap-2 mt-2 w-[80%] ml-auto">
                {messages[messages.length - 1].recommendations?.map((recommendation, index) => (
                  <button
                    key={index}
          ...
          {/* <div className="text-sm text-gray-500 mb-2">WebSocket: {connectionStatus}</div> */}
          {inputEnabled && (
